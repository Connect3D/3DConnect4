...
public class CommandParser {
	...
	public Pair<Command, String[]> parse(String input) 
		...
		// all whitespace is now reduced to single spaces
		String command = Util.join(splittedCommand);			
		// throws unsupported exception if not recognized
		Command.Type type = Command.getType(splittedCommand, direction);	
		
		switch (type) {
			case ACTION:
				Action action = Action.parse(splittedCommand);
				if (Action.isValid(action, command, direction)) {
					return new Pair<Command, String[]>(action, 
							Arrays.copyOfRange(splittedCommand, 1, splittedCommand.length));
				}
				break;
			case ACKNOWLEDGEMENT:
				...
				if (Acknowledgement.isValid(acknowledgement, command, direction)) {
					return new Pair<Command, String[]>(acknowledgement,
							Arrays.copyOfRange(splittedCommand, 1, splittedCommand.length));
				}
				break;
			case EXIT:
			...
			case ERROR:
				...
				if (Error.isValid(error, command, direction)) {
					return new Pair<Command, String[]>(error,
							Arrays.copyOfRange(splittedCommand, 2, splittedCommand.length));
				}
				break;
		}
		return null;
